...
AWARDS_DIR = "static/awards"
INVENTORY_DIR = "static/inventory"
CREDS_SCOPES = [
...
LANGUAGES = {
    ...
    "Indonesia": {
        ...
        "cost_components": "Komponen Biaya",
...
def save_image(uploaded_file, directory=IMAGE_DIR):
    """Save uploaded image with unique filename and return relative path"""
    try:
        ...
        return image_path.replace('\\', '/')  # Ensure consistent path format
    except Exception as e:
...
def display_image(image_path, width=300):
    ...
    try:
        if not image_path or image_path == DEFAULT_IMAGE or not os.path.exists(image_path):
        ...
        if not os.path.isabs(image_path):
            # Try to find the image in the static directory
            possible_paths = [
                image_path,
                os.path.join(IMAGE_DIR, image_path),
                os.path.join(ACTIVITIES_DIR, image_path),
                os.path.join(AWARDS_DIR, image_path),
                os.path.join(INVENTORY_DIR, image_path)
            ]
...
def award_card(award, language):
    ...
    try:
        with st.container(border=True):
            # Award Image - PERBAIKAN: Hapus use_container_width
            image_path = award.get("image_path", DEFAULT_IMAGE)
            display_image(image_path)  # Hanya parameter image_path saja
